...
* В момента, в който деполойнем нещо на мрежа, понеже всички записи са immutable, не можем да го манипулираме

* **_Можем обаче да го блокираме със `selfdestruct` примитива_**

  - За да не остане ресурсът блокиран, можем да посочим адрес, на който да се изпрати останалия ресурс, независимо дали има `payable` функции
...
- използва се, за да си взимодействаме със СК, предоставя интерфейс към функциите, които ни интересуват
...
* предложения как да бъде развит ethereum по накакъв начин
...
- race attack поражда race condition
  - пускаме транзакция, която е възможно най-малко атрактивна за валидаторите(стои си в pool-а известно време)
...
* Commitment schemes over revealing information directly
  ...
  - самият СК не трябва да дава повече информация за СК, отколкото е нужно
...
❗ infura - услуга, един node за публична употреба, ако нямаме node няма как да се вържем с мрежата и да inteact-ваме с протокола
